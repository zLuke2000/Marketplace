...
export async function getMyProducts() {
	...
	}).catch((error) => console.error('Error occurred while trying to read the products!', error))
	.finally(() =>  HTML.hideSpinner('#myProductsRow') );
}
...
export async function resellProduct(btn, price) {
	...
	fetch('/resell-product', {
		...
		if (res.ok) {
			...
			alert('Il prodotto \u00E8 ora disponibile sul mercato!');
		} else { console.error('Something went wrong while fetching!', res.status) }
...
function getProducts(url, obj) {
	...
	}).catch((e) => console.error('An error occurred while fetching the products!', e))
	.finally(() => HTML.hideSpinner('#buyProductsRow'));
}